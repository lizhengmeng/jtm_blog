...
<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
    ...
    <td>
        <?php if($v['is_original'] == 1): ?>✔<?php else: ?> ✘<?php endif; ?>
    </td>
    <td>
        <?php if($v['is_show'] == 1): ?>✔<?php else: ?> ✘<?php endif; ?>
    </td>
    <td>
        <?php if($v['is_top'] == 1): ?>✔<?php else: ?> ✘<?php endif; ?>
    </td>
    <td>
        <?php echo ($v['click']); ?>
    </td>
    ...
    <td>
        <a href="<?php echo U('Admin/Article/edit',array('aid'=>$v['aid']));?>">修改</a> | <a href="javascript:if(confirm('确定要删除吗?')) location='<?php echo U('Admin/Recycle/recycle',array('table_name'=>'Article','id_name'=>'aid','id_number'=>$v['aid']));?>'">删除</a>
    </td>
